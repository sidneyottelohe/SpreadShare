...
class FeedSideMenu extends Component {
  ...
  isFilter(filter) {
    return this.props.viewStore.filter === filter ? 'button is-checked' : 'button'
  }
  isSort(sort) {
    return this.props.viewStore.sort.property === sort ? ' is-checked' : ''
  }
  render() {
    return (
          ...
          <div className="filter-menu-wrap w-hidden-small w-hidden-tiny" id="search">
            <div className="w-embed">
              <input
                ...
                value={this.props.viewStore.search} />
            </div>
          </div>
